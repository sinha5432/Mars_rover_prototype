import rospy
import time
from kalman_filter.msg import kalman_filtered_values
...
def origin(msg):
	global lat_org,log_org
	lat_org = msg.x
	log_org = msg.y

def shifting_centre(msg):
    global xc , yc
    xc = (-1)*msg.x
    yc = (-1)*msg.y

def get_lat_long_based_on_x_y(msg,pub1):
	global lat_org,log_org,xc,yc,x,y
	...
	q = NavSatFix()
   	...
   	y = y - yc
    	...
    	lat,log = xy2ll(x, y, lat_org, log_org)
	q.latitude = lat
	q.longitude = log
	pub1.publish(q)
...
if __name__ == '__main__':
	print("In main")
	rospy.init_node('gps_converter1')
	print("Enter the Goal Location GPS coordinates")

	pub1 = rospy.Publisher('/filtered_gps' , NavSatFix , queue_size = 50)
	sub = rospy.Subscriber('lat_long' , kalman_filtered_values , origin )
        sub1 = rospy.Subscriber('/covarience_matrix' , covarience_values , shifting_centre )
	sub2 = rospy.Subscriber('filtered_distance' , kalman_filtered_values , get_lat_long_based_on_x_y , pub1)

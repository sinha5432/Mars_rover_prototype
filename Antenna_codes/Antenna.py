...
def get_xy_based_on_lat_long(msg,distance_pub):
	...
	xg2,yg2 = ll2xy(p_lat,p_lon,d_lat,d_lon)#ll2xy???
	
	xa,ya = ll2xy(p_lat,p_lon,d_lat,d_lon)#ll2xy???

	
	rospy.loginfo("LAT COORDINATES ==>"+str(p_lat)+","+str(p_lon))
...
if __name__ == '__main__':
	print("In main")
	rospy.init_node('antenna_gps_converter')
	print("Enter the Goal Location GPS coordinates")
	p_lat=float(input("Enter antenna Latitude:"))
	p_lon=float(input("Enter antenna Longitude:"))
	distance_pub = rospy.Publisher('/distance',Pose,queue_size=1)
	rospy.Subscriber('/navsat/fix',NavSatFix,get_xy_based_on_lat_long,distance_pub)
